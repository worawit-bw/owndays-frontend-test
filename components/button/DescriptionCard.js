const DescriptionCard = ({ title, description, price, hightLight = false }) => {
  return (
    <div className="group flex flex-wrap flex-col bg-white border-black rounded-lg border-2 p-5.5 pl-5.5  lg:py-8.5 lg:px-9.5 max-w-[366px] min-h-[230px] box-shadow-custom hover:!shadow-none relative cursor-pointer">
      {/* Title */}
      <h2 className="text-[16px] md:text-[18px] font-semibold mb-3 text-black tracking-[2px] text-left font-mizolet">{title}</h2>

      {/* Description */}
      <p className="text-[12px] md:text-[13px] leading-6 mb-2.5 text-black text-left font-black font-bold font-yu-gothic">
        {description}
      </p>
      ...
      <div className="hidden sm:flex-1 flex-col px-0 md:px-3 py-3.5 md:py-4.5 text-left">
          {/* Text left */}
      ...
      {/* Arrow Right */}
      <div className={`sm:hidden flex items-center justify-center ${!title && 'w-full'} mb-[12px] sm:mb-0 sm:w-max`}>
